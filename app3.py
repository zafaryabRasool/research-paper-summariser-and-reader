...
if uploaded is not None:
    ...
    with left:
        ...
        viewer_url = f"http://localhost:{STATIC_PORT}/pdfjs/web/viewer.html?file=data:application/pdf;base64,{pdf_base64}"
        ...
        for idx, para_obj in enumerate(paragraphs, start=1):
            with st.expander(f"Paragraph {idx}"):
                ...
                if st.button(f"Summarize {idx}"):
                    ...
                    viewer_url = f"http://localhost:{STATIC_PORT}/pdfjs/web/viewer.html?file=data:application/pdf;base64,{highlighted_pdf_base64}"
                    iframe = f'<iframe src="{viewer_url}" width="100%" height="600" style="border:none;"></iframe>'
                    html(iframe, height=600)
